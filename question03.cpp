...
#include <memory>
/*explanation
The memory leak happend because we alloceted Payload and never deleted it, that will not be enough to just delete it in line 58 because we still using it
 and join() is not allowd.
using shared_ptr (smart ptr) to make sure payload will be deleted when we do need it anymore.
*/
...
void dispacher_thread() {
	...
	std::shared_ptr<Payload> payload =std::make_shared<Payload>(Payload(1));
